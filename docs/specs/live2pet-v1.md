...
- Discover candidate Source Packages up to two levels below a selected local folder or public GitHub tree folder. The complete Models destination accepts local folder drops. GitHub discovery must not clone the repository or download model blobs until a candidate is selected or the user explicitly chooses **Download all**.
...
- Selected or explicitly batch-downloaded GitHub models use a bounded App-private LRU cache. **Download all** preflights the complete detected set against the configured limit, exposes determinate model-count progress, continues after individual download failures, and reports its result. Model-card thumbnails remain viewport-lazy. Settings shows usage, allows a 256 MiB–20 GiB limit, and provides an explicit clear action. The default is 1 GiB; one model may not exceed 4 GiB or 10,000 files.
...
- The center column is the only visible source preview and supports Motion play, pause, restart, loop, speed control, Expression apply/clear when the renderer exposes Expressions, and an explicit reset that recreates the renderer while retaining saved project configuration. Spine skins are not labeled as Expressions; V1 may use the default Spine skin.
- Preview fits the visible animated model bounds instead of silently cropping it.
- A searchable Visibility drawer beside the preview lists Live2D Parts in their collapsible parent hierarchy, modern Cubism root Drawables in an Unattached meshes group, or Spine Slots as renderer-neutral Visual Elements. Search preserves matching ancestor paths. It provides per-element and unattached-group show/hide, temporary Solo, and Restore all actions.
...
- No Cubism Core, legacy runtime, optional Spine renderer pack, model, texture, or copyrighted example is bundled in source artifacts. Public binary or renderer-pack distribution remains separately gated by third-party terms.

### Project and mapper

- The Map destination remains a three-column workflow: Motion/Expression library, live preview with project-specific Visibility, and target mapping.
...
- Safe independent encoding and target assembly work may run in parallel. Source capture remains bounded by renderer stability rather than an arbitrary worker count.
...
Open a permitted Live2D model with a removable background Part. Hide that Part from the Map preview, confirm the visible bounds reframe around the remaining character, save and reopen the project, and confirm the hidden identity persists. Build both targets and verify that their generated assets match the preview visibility. Restore all elements and confirm the affected capture cache is invalidated without discarding unrelated entries.
...
- Shared renderer contract tests cover playback, Visual Element enumeration/application, visible bounds, deterministic stepping, RGBA capture, unload, and failure isolation.
- Spine pack tests cover explicit consent, fixed-version resolution, maximum size, integrity failure, atomic installation, reuse, removal, and unsupported-version recovery without requiring a network download in public CI.
...
- Multiple Spine runtime lines, `.spine` editor projects, arbitrary runtime URLs, unknown Spine containers, automatic first-run Spine downloads, advanced Spine skin authoring, and attachment-level editing.
...
- Do not commit or publish example models, generated character themes, Cubism Core, legacy runtimes, or Spine renderer artifacts without explicit redistribution permission.
- Live2Pet's Apache-2.0 license does not relicense the Spine Runtime. Public binary or optional-pack distribution requires a recorded review of the Spine Runtime License and any Spine Editor license required by the exact packaging model.
...
- Optional Spine and shared visibility decision: `docs/adr/0014-use-optional-versioned-spine-renderer-packs-and-shared-visual-settings.md`
