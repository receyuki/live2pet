...
The development Electron shell loads the shared Mapper from a fixed repository path, denies new-window and renderer navigation, rejects webviews and permission requests, and exposes only typed `live2pet:app` IPC methods through a sandboxed, context-isolated preload. The main process can inject the shared `buildProjectTargets` service behind `buildProject`; the typed response carries progress, binary-free build summaries, and short-lived artifact handles, while `getBuildArtifact` is an explicit download step and installation stays separate. The Mapper document carries a local CSP that permits only its packaged scripts, inline prototype UI, local images, user-selected runtime blobs, and loopback Mapper Sessions. `apps/desktop/scripts/stage-mapper-assets.cjs` can produce a self-contained local Mapper bundle with pinned Pixi, pixi-live2d-display, and zip.js assets plus hashes and license notices; the packaged shell resolves that bundle from `process.resourcesPath`. It does not package Cubism Core, model content, example assets, or generated packages. Forge makers, signed installers, and the staging step's clean-machine release check remain release-gated work.
