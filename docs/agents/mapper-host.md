...
This boundary is intentionally separate from Electron IPC. The App shell contains a Live2D renderer-realm host under `apps/desktop/renderer-host.cjs`: it creates a dedicated BrowserWindow with Node integration disabled, context isolation, sandboxing, strict CSP, and a loopback-only Source Package/runtime server. The main Mapper still exposes only typed preload methods and never receives model/runtime bytes. Renderer process, page-bootstrap, and unknown adapter failures unload the adapter and destroy the isolated realm; `restart()` creates a fresh generation. The App IPC layer still exposes the existing one-session renderer commands, but the 2026-09-01 V1 rescope makes this an internal capture, recovery, and diagnostic seam. It must not create a second user workflow. The center column is the only V1 Source Package preview for both standard directories and supported PCK inputs, and the current **Open isolated preview** controls are scheduled for removal from the normal UI. ADR-0011 makes the Pixi/Cubism bridge the personal-use V1 production path; the official modern Web Framework bridge remains experimental and post-V1.
...
The source-preview panel must expose Pause, Resume, Restart, Loop, and playback-speed controls directly on the center-column preview. The existing typed renderer commands may support the same internal renderer contract, but V1 cannot require a user to open or manage a separate renderer window to obtain control parity.
