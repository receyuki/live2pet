...
The project service now also provides explicit autosave recovery, source relinking, and review gating when a source fingerprint changes. The Electron mapper still needs to connect these operations to its UI and build flow. The desktop shell now has a repeatable `prepare:mapper` staging step that rewrites workspace-relative browser dependencies into a local bundle for future packaging; Core and legacy runtimes remain user-provided.

### Current implementation update (2026-08-31)

The Mapper now exposes a shared-App Clawd Theme ZIP path that captures mapped Motion frames, produces a guide-shaped `theme.json`, validates the package, and returns an explicit artifact download handle. It also ships the first English/Chinese (`zh-CN`) i18n key layer and locale selector. Browser-only sessions keep the Clawd build action disabled because they do not provide the trusted shared encoder; target behavior preview and installation remain downstream.
...
Current implementation update: the App-host seam now exercises a synthetic Clawd build end to end. The remaining WP5 work is target behavior preview, install UX, and real clean-machine acceptance.
