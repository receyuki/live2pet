...
Current implementation update: the App-host seam now exercises a synthetic Clawd build end to end, and Mapper can preview generated WebP state/reaction assets with manifest fallback resolution. The remaining WP5 work is full target behavior simulation, install UX, and real clean-machine acceptance.
...
Current implementation update: the shared Mapper build controls now cover both Codex Pet and Clawd Theme artifacts, generated target previews are visible after successful builds, and the initial English/Chinese locale layer is wired into the UI without changing IPC or target schemas. The remaining App work is packaging, full target behavior previews, installation controls, and release qualification.
