...
const { captureBatchSize } = require('./capture-batch.cjs');
...
async function sampleMotionCandidates(renderer, options = {}) {
  ...
  const candidates = [];
  const metrics = { captureRoundtripMs: 0, captureRequests: 0, captureBatchCalls: 0, alphaBoundsMs: 0, candidateAnalysisMs: 0, rgbaBytes: 0 };
  let previousRgba = null;
  let previousBounds = null;
  try {
    const batchSize = options.captureBatch !== false && renderer.supportsCaptureBatch !== false && typeof renderer.captureRgbaBatch === 'function' ? captureBatchSize(width, height) : 1;
    ...
    for (let index = 0; index < samples;) {
      ...
      const times = Array.from({ length: Math.min(batchSize, samples - index) }, (_, offset) => duration * (samples === 1 ? 0 : (index + offset) / (options.includeEndpoint === false ? samples : samples - 1)));
      const captureStarted = performance.now();
      metrics.captureRequests += 1;
      if (useBatch) metrics.captureBatchCalls += 1;
      const captures = useBatch
        ? await renderer.captureRgbaBatch({ width, height, motionId, times })
        : [await renderer.captureRgba({ width, height, motionId, time: times[0] })];
      metrics.captureRoundtripMs += performance.now() - captureStarted;
      if (options.signal?.aborted) fail('BUILD_CANCELLED', 'Package Build was cancelled.');
      if (!Array.isArray(captures) || !captures.length || captures.length > times.length) fail('INVALID_RENDER_CAPTURE', `Renderer returned an invalid capture batch for ${motionId}.`);
      for (const [offset, capture] of captures.entries()) {
        if (options.signal?.aborted) fail('BUILD_CANCELLED', 'Package Build was cancelled.');
        ...
        const scores = options.candidateScoring === false ? {} : { visualChange: rgbaDifference(previousRgba, rgba), boundsDelta: boundsDifference(previousBounds, bounds) };
        candidates.push({ id: `${motionId}#${index}`, time, bounds, ...scores, width, height, rgba: new Uint8Array(rgba) });
        metrics.candidateAnalysisMs += performance.now() - analysisStarted;
        if (options.candidateScoring !== false) {
          previousRgba = rgba;
