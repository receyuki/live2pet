...
- `apps/mapper/` — browser-based Live2D motion preview, Clawd mapping, Codex nine-row mapping, local Codex ZIP fallback build, and shared-App Clawd Theme ZIP build. The Mapper has no runtime CDN dependency: modern Core can be selected locally, and Cubism 2 preview requires a user-selected local `live2d.min.js`. Its first English/Chinese (`zh-CN`) locale layer is presentation-only and does not change project or package schemas.
