...
在映射页通过“动作与表情 / 显示与隐藏”两个 Tab 切换。在“显示与隐藏”中搜索模型部件，逐项选择“隐藏 / 显示”。可以用“单独查看”辨认部件，或点击“全部恢复”取消隐藏。App 不会根据名称自动隐藏内容；模型提供名称时优先显示名称，否则显示原始 ID。现代 Cubism 中没有归属任何 Part 的根级 ArtMesh 会标记为“未归属网格”，因此也可以单独预览和隐藏。
