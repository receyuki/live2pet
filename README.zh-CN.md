...
在**映射 → 可见性**中搜索模型部件，逐项选择“隐藏 / 显示”。可以先用“单独查看”辨认部件，或点击“全部恢复”取消隐藏。App 不会根据部件名称自动隐藏内容；模型提供名称时优先显示名称，否则显示原始部件 ID。

可以按需查看某个部件在当前姿势下的独立外观缩略图，不会一开始就生成整张列表的预览。没有可见像素的部件会说明当前姿势下没有内容。

隐藏设置保存在项目中，并同时用于预览和两种目标构建。开关部件只更新当前画面；完整动作取景检查仅在需要捕获时进行。“单独查看”只是临时预览，不会保存。旧项目默认显示全部部件。若背景与角色画在同一网格里，此功能无法将其拆开。
