...
「模型」页统一提供打开项目、导入资源和浏览模型库。拖入模型文件夹会将其作为模型库打开，拖入 PCK 文件则直接导入；无论 Electron 将文件夹表示为目录本身还是其中的多个文件，都能正确识别。这样包含多个模型的集合不会再被误判为一个 Source Package。优先使用本地模型附带的 `preview`／`cover` 图片，没有封面时使用已安装的运行时生成 256 像素缩略图。可见卡片优先排队，屏幕外的卡片随后会在后台自动补齐；最多同时渲染两个缩略图，单个渲染超过 15 秒会停止等待，不会卡住 App 或阻塞整个模型库。缩略图内存缓存最多 16 MiB，退出 App 后释放。缺少运行时会提示，不会自动下载。点击卡片可实时预览并选择动作，只有点击「使用这个模型」才会替换当前项目。切换到映射或构建后，模型库仍会保留。
