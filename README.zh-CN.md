...
「模型」页统一提供打开项目、导入资源和浏览模型库。优先使用本地模型附带的 `preview`／`cover` 图片，没有封面时使用已安装的运行时生成 256 像素缩略图。只处理进入可视区域的卡片，并逐个生成；缩略图内存缓存最多 16 MiB，退出 App 后释放。缺少运行时会提示，不会自动下载。点击卡片可实时预览并选择动作，只有点击「使用这个模型」才会替换当前项目。切换到映射或构建后，模型库仍会保留。
