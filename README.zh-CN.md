...
3. 在“模型”页点击卡片预览，选好后点击“使用这个模型”。当前模型的可展开详情保留版本、动作、表情与警告；受支持的 Spine 资源缺少渲染包时会提供“安装 Spine 支持”。
...
「模型」页统一提供打开项目、导入资源和浏览模型库。可以拖入单个模型文件夹或单个 PCK 文件；无论 Electron 将文件夹表示为目录本身还是其中的多个文件，都能正确识别。优先使用本地模型附带的 `preview`／`cover` 图片，没有封面时使用已安装的运行时生成 256 像素缩略图。只处理进入可视区域的卡片，并逐个生成；单个渲染超过 15 秒会停止等待，不再阻塞后续模型。缩略图内存缓存最多 16 MiB，退出 App 后释放。缺少运行时会提示，不会自动下载。点击卡片可实时预览并选择动作，只有点击「使用这个模型」才会替换当前项目。切换到映射或构建后，模型库仍会保留。
