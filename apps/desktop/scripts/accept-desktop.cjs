...
(async () => {
  ...
  for (const [generation, source] of [['modern', process.env.LIVE2PET_MODERN_SOURCE], ['legacy', process.env.LIVE2PET_CUBISM2_SOURCE]]) {
    ...
    if (hiddenElementId) {
      ...
      const thumbnail = await page.evaluate(id => window.live2pet.getPreviewVisualElementThumbnail({ id }), hiddenElementId);
      assert.equal(thumbnail.ok, true, JSON.stringify(thumbnail));
    ...
    await page.getByRole('button', { name: 'Save project', exact: true }).click();
    await page.getByText('Saved', { exact: true }).waitFor();
    assert.deepEqual(JSON.parse(fs.readFileSync(file)).visualSettings.hiddenElementIds, [], 'source fingerprint replacement resets old-model Part IDs');
    await page.reload();
    await page.getByRole('button', { name: 'Open project', exact: true }).click();
    await page.getByRole('region', { name: 'Review changed Source Package', exact: true }).waitFor();
