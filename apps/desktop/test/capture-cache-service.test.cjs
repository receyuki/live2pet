...
test('capture does not reuse frames recorded before fixed-rate physics integration', async () => {
  const { cache, service } = setup();
  const old = createCaptureCacheService({ cache, rendererVersion: 'pixi-live2d-capture-v6', getRuntimeForGeneration: async () => ({ descriptor: { fingerprint: 'a'.repeat(64) } }) });
