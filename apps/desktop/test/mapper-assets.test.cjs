...
test('mapper asset staging creates a self-contained local document without Core', () => {
  ...
  try {
    ...
    assert.match(html, /src="\.\/build-progress\.js"/);
    assert.match(html, /src="\.\/clawd-capture-plan\.js"/);
    assert.match(html, /src="target-profiles\/clawd\.js"/);
    assert.match(html, /src="target-profiles\/codex-pet\.js"/);
    assert.ok(fs.statSync(path.join(output, 'build-progress.js')).isFile());
    assert.ok(fs.statSync(path.join(output, 'clawd-capture-plan.js')).isFile());
    assert.ok(fs.statSync(path.join(output, 'target-profiles/clawd.js')).isFile());
    assert.ok(fs.statSync(path.join(output, 'target-profiles/codex-pet.js')).isFile());
    assert.deepEqual(manifest.supportFiles.map((file) => file.path), ['build-progress.js', 'clawd-capture-plan.js', 'target-profiles/clawd.js', 'target-profiles/codex-pet.js']);
    for (const file of manifest.supportFiles) assert.match(file.sha256, /^[a-f0-9]{64}$/);
    assert.equal(fs.existsSync(path.join(output, 'renderer.html')), false);
