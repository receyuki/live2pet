...
function createPlannedBuildService({ getCache, resolveContext, buildProject }) {
  return async function plannedBuild(input = {}) {
    ...
    for (const target of targets) {
      ...
      if (!context) context = await resolveContext(input.project);
      ...
      const store = (key, bytes) => cache.put(key, bytes, { projectId: input.project.projectId, sourceFingerprint: input.project.source.fingerprint, artifact: 'planned-encoded-webp' });
      if (target === 'codex-pet') {
