...
export function ModelPreview({ library, candidate, locale, onUse, onClose, direct, onConfigureRuntime }: { library?: SourceLibrary; candidate: SourceLibraryCandidate; locale: Locale; onUse: (motion: string) => Promise<void>; onClose: () => void; direct?: SourceLibrarySelection; onConfigureRuntime?: () => void }) {
  ...
  const { status, run: runPreview } = usePreviewSession({
    surface, enabled: Boolean(selection && hasPreviewApi()), projectId: 'library-preview',
    sourceFingerprint: selection?.inspection.source.fingerprint, retry, allowMinimumBounds: true,
  });
  const ready = status?.state === 'ready';
  const motions = status?.catalog?.motions ?? inspection?.motions ?? [];
  useEffect(() => {
    setSelection(null); setError(''); setMotion(''); setExpression('');
    let active = true;
    void (async () => {
      try {
        ...
        if (active) {
          setSelection(result);
          if (!hasPreviewApi()) setError(translate(locale, 'previewDesktopRequired'));
        }
      } catch (cause) { if (active) setError(cause instanceof Error ? cause.message : String(cause)); }
    })();
    return () => { active = false; };
  }, [library?.libraryId, candidate.id, direct, retry]);
  useEffect(() => {
    if (status?.state === 'failed') setError(status.error?.message || translate(locale, 'libraryPreviewUnavailable'));
    if (!ready || !selection) return;
    const catalog = status?.catalog?.motions ?? selection.inspection.motions;
    setMotion(current => current || catalog[0]?.id || '');
  }, [ready, status?.state, status?.catalog, selection, locale]);
  useEffect(() => {
    if (ready && motion) void runPreview(() => playLive2DPreview({ motionId: motion, loop: true })).catch(cause => setError(String(cause)));
  }, [ready, motion, runPreview]);
  return <aside className="library-detail" aria-label={candidate.name} data-tour-id="model-preview">
    ...
    {ready && <label>{translate(locale, 'sourceMotions')}<select aria-label={translate(locale, 'sourceMotions')} value={motion} onChange={event => { setMotion(event.target.value); }}>{motions.map(item => <option key={item.id} value={item.id}>{item.name || item.id}</option>)}</select></label>}
    {ready && !!inspection?.expressions.length && <label>{translate(locale, 'sourceExpressions')}<select aria-label={translate(locale, 'sourceExpressions')} value={expression} onChange={event => { const value = event.target.value; setExpression(value); void runPreview(() => setLive2DPreviewExpression(value || null)).catch(cause => setError(String(cause))); }}><option value="">—</option>{inspection.expressions.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}
