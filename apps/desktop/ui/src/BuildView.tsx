          ...
          return (
            <Card className="surface-card build-card" key={target}>
              <Card.Content>
                ...
                {artifact && current.artifactSnapshot && current.artifactSnapshot !== JSON.stringify(project) && <p role="status" className="build-feedback">{t('buildArtifactStale')}</p>}
                {artifact && current.status !== 'succeeded' && <p role="status" className="build-feedback">{t('buildArtifactPrevious')}</p>}
