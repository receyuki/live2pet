...
import { Eye, EyeOff, Image as ImageIcon, ImageOff, X } from 'lucide-react';
...
export function VisibilityPanel({ locale, elements, settings, soloId, busy, thumbnail, onSettings, onSolo, onInspect, onClose }: {
  ...
  onSettings: (settings: VisualSettings) => void; onSolo: (id: string | null) => void; onInspect: (id: string) => void; onClose: () => void;
}) {
  ...
  return <section className="workspace-panel visibility-panel" aria-label={t('visibility')}>
    ...
    <div className="motion-list">
      {rows.map(element => <div className="visibility-row" key={element.id}>
        <span className="grow-copy"><strong title={element.name}>{element.name}</strong><small title={element.id}>{element.id}</small></span>
        <div className="visibility-actions"><Button isIconOnly size="sm" variant={thumbnail?.id === element.id ? 'secondary' : 'ghost'} isDisabled={busy} aria-label={`${t('inspectElement')} · ${element.name}`} aria-pressed={thumbnail?.id === element.id} onPress={() => onInspect(element.id)}><ImageIcon size={15} /></Button><Button isIconOnly size="sm" variant={hidden.has(element.id) ? 'ghost' : 'secondary'} isDisabled={busy} aria-label={`${t(hidden.has(element.id) ? 'showElement' : 'hideElement')} · ${element.name}`} aria-pressed={!hidden.has(element.id)} onPress={() => { const next = new Set(hidden); if (next.has(element.id)) next.delete(element.id); else next.add(element.id); onSolo(null); onSettings({ hiddenElementIds: [...next].sort() }); }}>{hidden.has(element.id) ? <EyeOff size={15} /> : <Eye size={15} />}</Button>
